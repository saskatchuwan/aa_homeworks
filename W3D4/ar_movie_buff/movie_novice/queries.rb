...
def pulp_fiction_actors
  ...
  Movie.joins(:actors).where('title = ?', 'Pulp Fiction').select('actors.id, actors.name')
end
...
def uma_movies
  ...
  Actor.joins(:movies).select('movies.id, movies.title, movies.yr').where('name = ?', 'Uma Thurman').order('yr ASC')
end
